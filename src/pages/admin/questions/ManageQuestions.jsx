...
import {
  ...
  QuestionFilters,
  QuestionStats
} from '../../Instructor/QuestionBank/components';
...
const ManageQuestions = () => {
  ...
  const {
    ...
    defaultValues,
    levelOptions
  } = useQuestions(subjects, 'admin'); // ✅ Truyền 'admin' vào hook
  ...
  // ✅ Table columns - THÊM CỘT "NGƯỜI TẠO" CHO ADMIN
  const columns = [
    {
      key: 'stt',
      title: 'STT',
      ...
      render: (value, item) => (
        <div className="creator-info">
          <div className="creator-name">
            {item.creatorId.fullName || 'N/A'}
          </div>
          <div className="creator-email">
            {item.creatorId.email || ''}
            <span className="creator-role">ROLE: {item.creatorId.role || 'N/A'}</span>
          </div>
    ...
    <div className="question-bank-container">
      ...
      <div className="question-bank-header">
        <div className="header-content">
          ...
          <p>Quản lý toàn bộ ngân hàng câu hỏi trong hệ thống</p>
        </div>
      ...
      <div className="table-section">
        <Table
          columns={columns}
          data={paginationData.paginatedQuestions.map((question, index) => ({ ...question, __stt: paginationData.startItem + index }))}
          loading={isLoading}
          emptyMessage={
            searchTerm || levelFilter !== 'all' || subjectFilter !== 'all'
              ? 'Không tìm thấy câu hỏi phù hợp với bộ lọc'
              : 'Chưa có câu hỏi nào trong hệ thống. Hãy tạo câu hỏi đầu tiên!'
          }
          responsive={true}
          hover={true}
          striped={true}
